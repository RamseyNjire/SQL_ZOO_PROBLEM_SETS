...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    JOIN
      stops AS end_stops
    ON
      end_routes.stop_id = end_stops.id
    WHERE
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    ...
    JOIN
      routes AS routesb
    ON
      routesa.company = routesb.company
    AND
      routesa.num = routesb.num
    JOIN
      stops AS start_position
    ON
      routesa.stop_id = start_position.id
    JOIN
      stops AS final_stop
    ON
      routesb.stop_id = final_stop.id
    WHERE
      start_position.name = 'Craiglockhart'      
  SQL
end
